package com.trade.securities.adapter.out.external.fsc

import com.trade.securities.application.port.out.LoadFscStockSubscriptionRightPort
import com.trade.securities.domain.FscStockSubscriptionRight
import org.slf4j.LoggerFactory
import org.eclipse.microprofile.config.inject.ConfigProperty
import jakarta.enterprise.context.ApplicationScoped
import com.trade.securities.infrastructure.RestClient
import com.trade.securities.infrastructure.UriComponentsBuilder

@ApplicationScoped
class FscStockSubscriptionRightAdapter(
    private val restClient: RestClient,
    @ConfigProperty(name = "fsc.api.key") private val apiKey: String,
    @ConfigProperty(name = "fsc.api.urls.subscription-right", defaultValue = "https://apis.data.go.kr/1160100/service/GetStockSecuritiesInfoService/getPreemptiveRightSecuritiesPriceInfo") private val apiUrl: String
) : LoadFscStockSubscriptionRightPort {

    private val log = LoggerFactory.getLogger(javaClass)

    override fun loadSubscriptionRights(
        pageNo: Int,
        numOfRows: Int,
        basDt: String,
        itmsNm: String?,
        likeItmsNm: String?
    ): Map<String, Any?> {
        return try {
            val builder = UriComponentsBuilder.fromUriString(apiUrl)
                .queryParam("serviceKey", apiKey)
                .queryParam("numOfRows", numOfRows)
                .queryParam("pageNo", pageNo)
                .queryParam("resultType", "json")
                .queryParam("basDt", basDt)

            itmsNm?.takeIf { it.isNotEmpty() }?.let { builder.queryParam("itmsNm", it) }
            likeItmsNm?.takeIf { it.isNotEmpty() }?.let { builder.queryParam("likeItmsNm", it) }

            val url = builder.build().toUriString()

            val response = restClient.get()
                .uri(url)
                .retrieve()
                .body(SubscriptionRightResponse::class.java)

            response?.response?.body?.let { body ->
                val items = body.items?.item?.map { mapToDomain(it) } ?: emptyList()
                mapOf(
                    "items" to items,
                    "totalCount" to body.totalCount,
                    "pageNo" to body.pageNo,
                    "numOfRows" to body.numOfRows
                )
            } ?: run {
                log.warn("FSC API (Subscription Right) returned empty or null for basDt: {}", basDt)
                mapOf("items" to emptyList<FscStockSubscriptionRight>(), "totalCount" to 0)
            }
        } catch (e: Exception) {
            log.error("Failed to fetch stock subscription rights from FSC", e)
            mapOf("items" to emptyList<FscStockSubscriptionRight>(), "totalCount" to 0)
        }
    }

    private fun mapToDomain(item: FscSubscriptionRightItem): FscStockSubscriptionRight {
        return FscStockSubscriptionRight(
            basDt = item.basDt,
            srtCd = item.srtnCd,
            isinCd = item.isinCd,
            itmsNm = item.itmsNm,
            mrktCtg = item.mrktCtg,
            clpr = item.clpr,
            vs = item.vs,
            fltRt = item.fltRt,
            mkp = item.mkp,
            hipr = item.hipr,
            lopr = item.lopr,
            trqu = item.trqu,
            trPrc = item.trPrc,
            lstDt = item.lstDt,
            lstPnt = item.lstPnt,
            hngpStrtDt = item.hngpStrtDt,
            hngpEndDt = item.hngpEndDt,
            stckSrtnCd = item.stckSrtnCd,
            stckItmsNm = item.stckItmsNm,
            stckClpr = item.stckClpr
        )
    }

    data class SubscriptionRightResponse(val response: com.trade.securities.adapter.out.external.fsc.dto.ResponseContent<FscSubscriptionRightItem>? = null)

    data class FscSubscriptionRightItem(
        val basDt: String? = null,
        val srtnCd: String? = null,
        val isinCd: String? = null,
        val itmsNm: String? = null,
        val mrktCtg: String? = null,
        val clpr: String? = null,
        val vs: String? = null,
        val fltRt: String? = null,
        val mkp: String? = null,
        val hipr: String? = null,
        val lopr: String? = null,
        val trqu: String? = null,
        val trPrc: String? = null,
        val lstDt: String? = null,
        val lstPnt: String? = null,
        val hngpStrtDt: String? = null,
        val hngpEndDt: String? = null,
        val stckSrtnCd: String? = null,
        val stckItmsNm: String? = null,
        val stckClpr: String? = null
    )
}
